...
df_gene <- GeneFeatureExtractor$new(gene_folder, pattern, feature)$parse_file_name_add_suffix()
...
for (top_n in c(30, 100, 250)) {
  ...
  data <- DataProcessor$new(diff_expr_file = file.path(data_dir,"diff_expr_KO - 5_g vs. PBS.csv"), 
                            ...
                            pattern = pattern)
  df_diff_expr <- data$extract_top_n_feature(top_n = top_n, 
                                             pivot = T,
                                             feature = feature)
...
df_diff_expr_unpivot <- data$extract_top_n_feature(top_n = top_n, 
                                                   pivot = F,
                                                   feature = feature)
